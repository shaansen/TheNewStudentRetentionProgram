...
function processOHData(oh, columns) {
	...
	var result = TAdata.map(function(d) {
		...
		if (TAMappings[d["TA who helped"]] == undefined) {
			TAMappings[d["TA who helped"]] = makeRandomNames(
				d["TA who helped"]
			);
		}
		return {
			Username: nameMappings[d["Email Address"]],
			...
			"TA Name": TAMappings[d["TA who helped"]],
	...
	csvFromGrades = data.slice(1, data.length).map(function(d, i) {
		...
		data[0].forEach(function(c, k) {
			if (c != "Username") {
				...
				nameMappings[d[k]] = makeRandomNames(d[k]);
				object[c] = nameMappings[d[k]] + "";
			}
		});
		return object;
	});
