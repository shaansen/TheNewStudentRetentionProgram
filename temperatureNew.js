...
function selectALineToViewOHData(currentLabel) {
	console.log("Selecting a line to view OH")
...
function getIntegratedCircles(currentLabel) {
	console.log("Get Integrated circles")
	var svg = d3.select(".viz-body").select("svg"),
		g = svg
			.append("g")
			.attr(
	...
	officeHourDots
		.append("circle")
		.filter(function(d, i) {
			console.log(d,currentLabel, d[0] == currentLabel)
	...
	officeHourDots
		...
		.attr("r", function(d) {
			if (d[3] != 0) return (10 * (d[3]-minRadius)) / maxRadius + 4;
			else return 0;
		})
		.style("fill", "none")
		// .style("stroke", "black");
